...
advent2_1 = (sum . map (getGameId) . filter (isValidGame (12,13,14))) <$> parse parseInput "advent2" input

advent2_2 = (sum . map (getGamePower . foldr (getMins) (Reveal 0 0 0) . getGameReveals)) <$> parse parseInput "advent2" input

-- Parser

parseInput :: Parser [Game]
parseInput = do
    games <- many1 parseGame
    return games
...
parseGame = do
    ...
    try endOfLine <|> (eof >> return ' ')
    return $ Game gameId reveals

parseReveals :: Parser [Reveal]
parseReveals = do
    reveals <- parseReveal `sepEndBy1` string "; "
    return reveals

parseReveal :: Parser Reveal
parseReveal = do
    colorCounts <- parseColorCounts `sepEndBy1` string ", "
    return $ Reveal (getCountOrDefault 'r' colorCounts) (getCountOrDefault 'g' colorCounts) (getCountOrDefault 'b' colorCounts)

parseColorCounts :: Parser (Int, Char)
parseColorCounts = do
...
getCountOrDefault _ [] = 0
getCountOrDefault clr1 ((c, clr2):xs)
    | clr1 == clr2 = c
    | otherwise = getCountOrDefault clr1 xs

-- Data Types

getGamePower (Reveal r g b) = r * g * b

getMins (Reveal r1 g1 b1) (Reveal r2 g2 b2) = Reveal r3 g3 b3
    where
        r3 = max r1 r2
        g3 = max g1 g2
        b3 = max b1 b2

isValidReveal (maxR, maxG, maxB) (Reveal r g b) = (r <= maxR) && (g <= maxG) && (b <= maxB)

isValidGame maxCubes (Game _ r) = foldr (&&) True $ map (isValidReveal maxCubes) r

getGameId (Game id _) = id
getGameReveals (Game _ r) = r

getRevealReds (Reveal r _ _) = r
getRevealGreens (Reveal _ g _) = g
getRevealBlues (Reveal _ _ b) = b

data Game = Game Int [Reveal] deriving Show

-- Reveal Red Green Blue
data Reveal = Reveal Int Int Int deriving Show

-- Input

example_input = [here|Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green|]

input = [here|Game 1: 1 red, 5 blue, 1 green; 16 blue, 3 red; 6 blue, 5 red; 4 red, 7 blue, 1 green
