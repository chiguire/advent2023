...
advent3_1 = sum . map (getPartNumber) $ filter (isPartValid (getMap theSchema)) $ getParts theSchema
    where
...
getSchema ipt = Schema { getParts = findParts theArray, getMap = theArray }
    where
        theArray = array ((0, 0), ((length $ head list) - 1, (length list) - 1)) flattenedList
        list = splitOn "\n" ipt
        enumeratedList = zip [0..] $ map (zip [0..]) list
        transposedList = [[((i, j), c) | (i, c) <- s ] | (j, s) <- enumeratedList]
...
findParts arr = foldr (++) [] [findPartsInLine arr j | j <- [0..(snd $ snd $ bounds arr)]]

findPartsInLine arr j = [ Part { getPartNumber = read (map (snd) p), getNeighbours = (findNeighbours (map (fst) p)) } | p <- getPartNumbers]
    where
        getPartNumbers = split (dropDelims . dropBlanks $ whenElt (\(_, c) -> not $ isDigit c)) (sortOn (fst . fst). filter (\((_, j1), _) -> j1 == j) . assocs $ arr)
        findNeighbours ixs@((i,j):_) = filter (coordWithinBounds) $ [(i-1, j), (i-1, j-1), (i-1, j+1)] ++ findNeighbours' ixs
        findNeighbours' ((i,j):[]) = [(i+1, j),(i+1, j-1),(i+1, j+1),(i, j-1), (i, j+1)]
        findNeighbours' ((i,j):ixs) = [(i, j-1), (i, j+1)] ++ findNeighbours' ixs
        coordWithinBounds (i, j) = (i >= 0) && (i <= (fst $ snd $ bounds arr)) && (j >= 0) && (j <= (snd $ snd $ bounds arr))

isPartSymbolAtPosition arr ix = isPartSymbol $ arr ! ix

isPartSymbol c = not (isDigit c || c == '.')

data Part = Part { getPartNumber :: Int, getNeighbours :: [(Int, Int)] } deriving Show

data Schema = Schema { getParts :: [Part], getMap :: Array (Int, Int) Char } deriving Show

-- Input

example_input = [here|467..114..
...*......
..35..633.
......#...
617*......
...
input = [here|.........798...145.........629....579.....455.....................130.............243.................154........167........................
